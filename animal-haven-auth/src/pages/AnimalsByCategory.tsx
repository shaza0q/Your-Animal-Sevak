...
const AnimalsByCategory = () => {
  ...
  const { farmId, animalType } = useParams<{ farmId: string; animalType: AnimalType }>();
    ...
    const init = async () => {
      try {
        ...
        const animalsData = await fetchFarmAnimals(
          ...
          page, 
          limit,
          appliedFilters.assigned === "all" ? undefined : appliedFilters.assigned === "true",
  ...
  if (loading && paginatedAnimals.length === 0) {
    return <LoadingState message={`Loading ${displayType.toLowerCase()}s for ${farm.name}...`} />;
  }
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {allAnimals.length} {allAnimals.length === 1 ? "animal" : "animals"} 
              </p>
            </div>
          </div>
        </div>
                ...
                <div className="grid gap-4 sm:grid-cols-3 mb-4">
                  {/* Assignment Filter */}
                   <div className="space-y-1.5">
                    <Label htmlFor="assigned" className="text-xs font-medium text-muted-foreground">
                      Assignment
                    </Label>
                    <Select
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      onChange={(e) => setFilters(prev => ({ ...prev, caretakerName: e.target.value }))}
                      className="h-9"
                    />
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      onChange={(e) => setFilters(prev => ({ ...prev, vetName: e.target.value }))}
                      className="h-9"
                    />
                  </div>
                </div>
                 ...
                 <div className="flex items-center gap-3 pt-3 border-t border-border/50">
                  ...
                  <Button 
                    ...
                    className={cn(
                      ...
                      (hasActiveFilters || hasPendingFilters) && "text-foreground hover:text-destructive"
                    )}
          ...
          <Card className="border-border/50">
            <CardContent className="p-12 text-center">
              <h3 className="text-lg font-semibold text-foreground mb-2">
                {hasActiveFilters ? "No Results Found" : `No ${displayType}s Found`}
              </h3>
                  ...
                  <CardContent className="pt-2 pb-4 px-4 space-y-2.5">
                    ...
                    <div className="flex items-center gap-2 text-sm">
                      {animal.caretakerId ? (
                        <>
                          ...
                          <span className="text-muted-foreground truncate">{animal.caretakerName}</span>
                        </>
                      ) : (
                    ...
                    {animal.veterinarianId && (
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        <span className="text-muted-foreground truncate">{animal.veterinarianName}</span>
                      </div>
                    )}
                   </CardContent>
                </Card>
              ...
              <div className="mt-6 py-3 px-4 bg-muted/30 rounded-lg border border-border/40">
                <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
                  ...
                  <div className="text-xs text-muted-foreground text-center order-first sm:order-none">
                    Page {page} of {totalPages} · Showing {startIndex + 1}–{Math.min(endIndex, totalItems)}
                  </div>
